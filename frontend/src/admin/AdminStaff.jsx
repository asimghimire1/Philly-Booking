...
// Mounts only while the modal is open (Modal renders null when closed), so the
// fields reset to the given member's values each time it opens.
function StaffForm({ member, onClose, onSubmit }) {
  ...
  const submit = (e) => {
    e.preventDefault()
    if (!name.trim()) return
    onSubmit({ name: name.trim(), gender, role: role.trim() || 'Therapist' })
    onClose()
  }
    ...
    <form onSubmit={submit}>
      <div className="border-b border-slate-100 px-6 py-5">
        ...
        <p className="mt-1 text-sm text-slate-500">
          {editing
            ? 'Update their details. Changes apply across the booking flow.'
            : "They'll be selectable in the booking flow."}
        </p>
      </div>
...
function IconButton({ onClick, label, tone = 'slate', children }) {
  const tones = {
    slate: 'text-slate-400 hover:bg-slate-100 hover:text-navy',
    rose: 'text-slate-400 hover:bg-rose-50 hover:text-rose-500',
  }
  return (
    <button
      type="button"
      onClick={onClick}
      aria-label={label}
      className={`shrink-0 rounded-full p-1.5 transition-colors ${tones[tone]}`}
    >
      {children}
    </button>
  )
...
export default function AdminStaff() {
  const { staff, addStaff, updateStaff, removeStaff, toggleStaff } = useAdminData()
  ...
  const submitForm = (data) => (editing ? updateStaff(editing.id, data) : addStaff(data))
  ...
  return (
    <div className="animate-step">
          ...
          <Card key={m.id} className={`p-5 ${m.active ? '' : 'opacity-60'}`}>
            <div className="flex items-start gap-3">
              ...
              <div className="flex shrink-0 items-center gap-0.5">
                <IconButton onClick={() => setEditing(m)} label={`Edit ${m.name}`}>
                  <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none">
                ...
                <IconButton onClick={() => setConfirmRemove(m)} label={`Remove ${m.name}`} tone="rose">
                  <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none">
      ...
      <Modal open={formOpen} onClose={closeForm}>
        <StaffForm member={editing} onClose={closeForm} onSubmit={submitForm} />
      </Modal>
