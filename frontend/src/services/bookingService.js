...
export async function submitBooking({ guests, details, precomputedPayload, bookingGroupId }) {
  const booked = guests.filter((g) => g.confirmed || guests.length === 1)

  // Use precomputed payload if provided (from context's completeBooking)
  const bookingsPayload = precomputedPayload || (() => {
